...
export class SearchEngine {
  ...
  async search(options: SearchOptions): Promise<SearchResult[]> {
    ...
    try {
      ...
      console.warn(`[SearchEngine] DuckDuckGo failed:`, (e as Error).message);
    }
    ...
    try {
      ...
      console.warn(`[SearchEngine] Bing failed:`, (e as Error).message);
    }
  ...
  private async searchDuckDuckGoHTML(query: string, maxResults: number): Promise<SearchResult[]> {
    ...
    try {
      ...
      const rawResults = await page.evaluate(() => {
        const items: Array<{ href: string; title: string; snippet: string }> = [];
  ...
  private async searchBing(query: string, maxResults: number): Promise<SearchResult[]> {
    ...
    try {
      await page.goto(
        `https://www.bing.com/search?q=${encodeURIComponent(query)}&count=${maxResults}&form=QBLH`,
