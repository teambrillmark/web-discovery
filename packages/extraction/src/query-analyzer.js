...
class QueryAnalyzer {
    ...
    detectLocation(query) {
        ...
        const cityMatch = query.match(/\b(Delhi|Mumbai|Bangalore|Chennai|Hyderabad|Pune|London|New York|Los Angeles|Chicago|San Francisco|Toronto|Sydney|Singapore)\b/i);
        return cityMatch?.[1] ?? null;
    }
    detectEntityType(query, intent) {
        if (intent === 'local_business_search')
            return 'local_business';
        if (intent === 'product_discovery')
            return 'product';
        if (/\bperson\b|\bfounder\b|\bceo\b|\bcto\b|\bpeople\b/i.test(query))
            return 'person';
        return 'company';
    }
    extractKeywords(query, isDomain) {
        if (isDomain) {
    ...
    buildObjective(query, intent, industry, isDomain) {
    ...
    generateSearchQueries(queryObj) {
        const queries = [];
